...
// //                     />
// //                     {/* Multi-stage vignette for premium depth */}
...
// //             {/* Glass Overlay Text — Refined to GOG Theme */}
...
// // const EventCard: React.FC<{ event: EventData; onClick: () => void; idx: number; is3D: boolean; sceneRotation: { x: number; y: number } }> = ({ event, onClick, idx, is3D, sceneRotation }) => {
// //     const cardRef = useRef<HTMLDivElement>(null);
// //     const videoRef = useRef<HTMLVideoElement>(null);
// //     const [isHovered, setIsHovered] = useState(false);
// //     const isIITMandi = event.id === 'iit-mandi-web3-blockchain';
...
// //         setIsHovered(true);
// //         if (isIITMandi && videoRef.current) {
...
// //                 perspective: "1200px",
// //                 zIndex: isHovered ? 50 : 1 // High z-index on hover to prevent overlapping
// //             }}
...
// //                     {isIITMandi && event.videoUrls ? (
// //                         <video
// //                             ref={videoRef}
// //                             src={event.videoUrls[0]}
...
// //                         <h3 className="text-base md:text-lg font-black text-white leading-tight tracking-tighter uppercase italic group-hover:text-[#34D562] transition-colors duration-400 mb-2">
// //                             {event.title}
// //                         </h3>
// //                         {/* <p className="text-[10px] md:text-xs text-white/50 font-medium leading-relaxed tracking-wide group-hover:text-white/80 transition-colors duration-400 line-clamp-2">
// //                             {event.tagline}
// //                         </p> */}
// //                     </div>
...
// //                             }}
// //                         >
// //                             <span className="text-[11px] tracking-tighter italic">
// //                                 {isIITMandi ? 'WATCH.PREVIEW' : 'LAUNCH.VIEW'}
// //                             </span>
// //                             <div className="p-2 bg-[#34D562] text-black shadow-[0_0_15px_rgba(52,213,98,0.5)]">
// //                                 {isIITMandi ? <Sparkles className="w-4 h-4" /> : <Eye className="w-4 h-4" />}
// //                             </div>
// //                         </motion.div>
// //                     </div>
// //                 </div>
...
// //     // 2-level 3D depth precisely as requested: Forward (40) and Base (0)
// //     const getCard3DTransform = (idx: number) => {
// //         const row = Math.floor(idx / 3);
// //         const zDepth = row % 2 === 0 ? 40 : 0;
// //         const scaleBase = row % 2 === 0 ? 1.02 : 1.0;
// //         return { z: zDepth, scale: scaleBase, row };
// //     };
...
// //                         >
// //                             {/* Cards container: Flex layout for centering incomplete last rows + balanced vertical gaps */}
// //                             <div className="flex flex-wrap justify-center gap-x-10 gap-y-32 transition-all duration-700">
// //                                 {eventsData.map((event, idx) => {
// //                                     const t = getCard3DTransform(idx);
// //                                     return (
// //                                         <div key={event.id} className="w-full md:w-[calc(50%-2rem)] lg:w-[calc(33.33%-2.5rem)] flex justify-center">
// //                                             <motion.div
// //                                                 initial={false}
// //                                                 animate={{
// //                                                     z: is3D ? t.z : 0,
...
// //                                                 }}
// //                                                 className="relative transition-all duration-500 hover:z-[100]"
// //                                             >
// //                                                 <EventCard
// //                                                     event={event}
// //                                                     idx={idx}
...
//     const getCard3DTransform = (idx: number) => {
//         const row = Math.floor(idx / 3);
//         const zDepth = row % 2 === 0 ? 80 : -30;  // alternate rows forward/back
//         const scaleBase = row % 2 === 0 ? 1.03 : 0.97;
...
//             {/* ═══════ Hero Section ═══════ */}
//             <section className="relative pt-36 md:pt-36 pb-8 md:pb-12 overflow-hidden">
...
//                         {/* Title */}
//                         <motion.h1
//                             className="text-3xl sm:text-4xl md:text-5xl lg:text-6xl xl:text-7xl font-bold leading-tight tracking-tight mb-6 whitespace-nowrap"
//                             initial={{ opacity: 0, y: 20 }}
//                             animate={{ opacity: 1, y: 0 }}
//                             transition={{ duration: 0.6, delay: 0.1 }}
//                         >
...
import GreenEmbers2 from '../components/ui/GreenEmbers2';
import ViewToggle from '../components/ViewToggle';
import { eventsData, EventData } from '../data/eventsData2';
                ...
                <div className="absolute inset-0 z-0 overflow-hidden">
                    {event.videoUrls && event.videoUrls.length > 0 ? (
                        <video
                            ref={videoRef}
                            src={event.videoUrls[0]}
                            className={`absolute inset-0 w-full h-full object-cover transition-opacity duration-500 z-0 ${isHovered ? 'opacity-100' : 'opacity-0'}`}
                            muted
                            loop
                            playsInline
                        />
                    ...
                    <img
                        src={event.cardImage}
                        alt={event.title}
                        className={`absolute inset-0 w-full h-full object-cover transition-all duration-1000 group-hover:scale-110 z-10 ${isHovered && event.videoUrls ? 'opacity-0' : 'opacity-60 group-hover:opacity-40'}`}
                    />
                    <div className="absolute inset-0 bg-gradient-to-t from-[#080808] via-[#080808]/40 to-transparent z-20" />
                </div>
...
const Events: React.FC = () => {
    ...
    return (
        <div className="min-h-screen bg-[#030303] text-white font-sans selection:bg-[#34D562] selection:text-black overflow-x-hidden">
            ...
            {/* ═══════ Hero Section ═══════ */}
            <section className="relative pt-36 md:pt-12 pb-8 md:pb-12 overflow-hidden">
                {/* Grid pattern overlay removed - handled by GridBackground */}
